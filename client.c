...
#define RETRY 10
//#define MAXPACKETSIZE 1024 // max number of bytes we can get at once 
...
int stopandwait(int sockfd, const void * packet, int expected_seqnum, int timeout, int flag ) {
...
/* Main Logic */
    ...
    struct stat finfo;
		...
		/* Send the filename... */
		send_results = stopandwait(sockfd, &packet, seqnum, 900000, ACK );
		if (send_results == -1) {
		...
		/* Send the filename... */
		send_results = stopandwait(sockfd, &packet, seqnum, 900000, FIN );
		if (send_results == -1) {
			fprintf(stderr, "Time out..\n");
			close(sockfd);
			return 2;
